...
public class StringArrayTest {
	...
	public static void main(String[] args) {
		...
		String[] strArray = {"continue","break","switch","array","print"};	
		//초기값 지정하며 배열 선언
...
//		String 클래스의 메소드 실행 : charAt(0), subString(2,5) 를 각 배열 요소 문자열로 실행합니다.
		...
		String[] names = new String[5];	// 지정된 문자열이 없으므로 값은 null(없다)
		for (int i = 0; i < names.length; i++) {
			System.out.println("i = "+i+",이름 = "+names[i]);
			//String은 클래스, 문자열은 객체 -> names 배열, names[i] 둘다 참조(주소)값을 지정합니다.
		}
//		System.out.println("charAt(0) : "+names[0].charAt(0) 
//				+",subString(2,5) : " +names[0].substring(2,5));	//문자열이 null일때 charAt 등 메소드 실행은 오류 발생
