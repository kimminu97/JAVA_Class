...
public class MyRedPenV4 {
	//47~59 라인 추가 : 틀린문제를 배열에 저장하고 파일쓰기 합니다.(MyFileRw.java 클래스 활용)
	// 전역변수
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			int ans;
			// 2)
			addList = new ArrayList<MathProblem>();
			...
			if (op == '0') { // 종료조건을 먼저 검사합니다.
				...
				System.out.println("----------------------------------------------");
				System.out.println("채점 합니다. 맞은 갯수  " + cnt + " ( " + cnt * (100 / num) + " 점)");
...
//				maths = addList.toArray(maths);
				...
				//틀린문제를 파일에 출력 : 파일명 yourmath.txt
				String filename = "D:\\developer_kmw\\dev\\File pract\\yourmath.txt";
				MyFileRW.fileWrite(filename, maths);
				for (MathProblem prob : addList) {
					if (!prob.isCorrect())
	...
	static int start(int num, char op) { // 리턴 int? 맞은개수, 인자 int num? 문제갯수, char op?연산자
		int cnt = 0;
		for (int i = 0; i < num; i++) {
			...
			// sc.nextInt(); 메소드 실행의 엔터는 다음 sc.nextLine()메소드에서 가져갑니다.->오류발생의 원인
