...
public class MyDictionaryApp { // 애플리케이션 클래스(main 메소드를 실행)
	// 전역변수 선언
...
//			String filename = "D:\\developer_kmw\\dev\\File pract\\mydict.txt";
			String filename;
			switch (sel) {
			case "1":
				wordAdd();
...
//					index = i;	//index 변수는 for문 밖에서 같은 단어의 인덱스 위치값을 활용할 때 사용하면됩니다.
	...
	private static void wordList() { // 단어목록
		for (Word w : mywords)
	...
	private static void wordRead(String filename) { // 파일에 저장된 단어들을 읽어옵니다.
		File file = new File(filename);
		try {
