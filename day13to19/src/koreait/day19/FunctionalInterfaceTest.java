...
//			자바에서는 람다식이라고도 부릅니다. (가독성, 간결성을 위해서 사용합니다.)
	...
	// 메소드이름은 생략하고 (인자선언) -> {메소드의 실행내용} 과 같은 형식으로 작성합니다.
		...
		//함수형 인터페이스를 익명클래스보다 더 간단한 형식으로 하는 람다식은 아래와 같습니다.
		...
		int result = fia.methodA(11, 12);
		System.out.println("result = " + result);
		
//		2) 람다식으로 객체를 생성. 추상메소드는 두개의 정수값을 범위로 하여 난수를 만들고 리턴
//			추상메소드가 여러개인 인터페이스는 람다식으로 못합니다. 익명클래스는 가능합니다.
		fia = (int a, int b) -> {Random r= new Random();
